...
import { DYE_DEBUG_TONE } from "@/lib/fluid-debug"
...
export default function OilShader({ settings = DEFAULT_FLUID_SETTINGS, onSupportChange }: OilShaderProps) {
  ...
  useEffect(() => {
    ...
    const debugDyeFrag = /* glsl */ `#version 300 es
    precision highp float;
    in vec2 v_uv;
    uniform sampler2D u_tex;
    out vec4 o;
    void main() {
      vec4 sampleValue = texture(u_tex, v_uv);
      float alpha = clamp(sampleValue.a, 0.0, 1.4);
      float boostedAlpha = pow(alpha, ${DYE_DEBUG_TONE.alphaGamma.toFixed(2)}) * ${DYE_DEBUG_TONE.alphaBoost.toFixed(2)};
      float signal = smoothstep(${DYE_DEBUG_TONE.minAlpha.toFixed(2)}, ${DYE_DEBUG_TONE.maxAlpha.toFixed(2)}, boostedAlpha);
      float rgbEnergy = clamp(length(sampleValue.rgb) * ${DYE_DEBUG_TONE.rgbWeight.toFixed(1)}, 0.0, 1.0);
      float glow = smoothstep(0.0, 1.0, signal * 0.82 + rgbEnergy * 0.35);

      vec3 shadow = vec3(${DYE_DEBUG_TONE.shadow.map((value) => value.toFixed(3)).join(", ")});
      vec3 mid = vec3(${DYE_DEBUG_TONE.mid.map((value) => value.toFixed(3)).join(", ")});
      vec3 highlight = vec3(${DYE_DEBUG_TONE.highlight.map((value) => value.toFixed(3)).join(", ")});
      vec3 tint = vec3(${DYE_DEBUG_TONE.tint.map((value) => value.toFixed(3)).join(", ")});

      vec3 color = mix(shadow, mid, signal);
      color = mix(color, highlight, glow * signal);
      color += rgbEnergy * tint;
    ...
    function drawTextureToScreen(source: FBO) {
      const { p, u } = progs.copy
      gl!.useProgram(p)
    ...
    function drawDyeDebugToScreen(source: FBO) {
      const { p, u } = progs.debugDye
      gl!.useProgram(p)
      gl!.uniform2f(u["u_texel"]!, source.texelX, source.texelY)
      gl!.uniform1i(u["u_tex"]!, source.attach(0))
      blit(null)
    ...
    function applyPointerSplats() {
      ...
      const dyeAmount = Math.min((0.18 + s * 8.0) * active.inputForce, 1.4)
    ...
    function step(dt: number) {
      const active = settingsRef.current
      const velocityDissipation = active.viscosity
      const dyeDissipation = active.dissipation
      const curlStrength = active.vorticity
      // 1) Curl
      {
      ...
      // 2) Vorticity
      {
        ...
        gl!.uniform1f(u["u_curlStrength"]!, curlStrength)
      ...
      // 7) Advect velocity
      {
        ...
        gl!.uniform1f(u["u_dt"]!, dt)
        gl!.uniform1f(u["u_dissipation"]!, velocityDissipation)
        blit(velocity.write)
        velocity.swap()
      }
      ...
      // 8) Advect dye
      {
        ...
        gl!.uniform1f(u["u_dt"]!, dt)
        gl!.uniform1f(u["u_dissipation"]!, dyeDissipation)
        blit(dye.write)
        dye.swap()
      }
    }
